...
class BaseRPC:
    ...
    async def write_as_json(self, stream, method, data, rpc_type, req_id=None):
        ...
        try:
            ...
            peer_id = getattr(stream.peer, 'rid', 'Unknown') if hasattr(stream, 'peer') else 'Unknown'
            self.config.app_log.warning(
            ...
            if hasattr(stream, "peer"):
                try:
                    await self.remove_peer(
                        stream,
                        reason="StreamClosedError during write_as_json"
                    )
            ...
            if hasattr(stream, "peer"):
                try:
                    await self.remove_peer(stream, reason="ConnectionResetError during write_as_json")
                except Exception as e:
                    self.config.app_log.error(f"Failed to remove peer after ConnectionResetError: {e}")
        except TimeoutError:
            self.config.app_log.warning(
            ...
            if hasattr(stream, "peer"):
                try:
                    await self.remove_peer(stream, reason="TimeoutError during write_as_json")
                except Exception as e:
                    self.config.app_log.error(f"Failed to remove peer after TimeoutError: {e}")
        except Exception as e:
            self.config.app_log.error(
            ...
            if hasattr(stream, "peer"):
                try:
                    await self.remove_peer(stream, reason=f"Exception during write_as_json: {e}")
                except Exception as e:
                    self.config.app_log.error(f"Failed to remove peer after exception: {e}")
        finally:
            if (
